...
@objc public extension UIImage{
    @objc static func dl_loadImage(_ name:String,module:String?=nil,bundleName:String? = nil) -> UIImage?{
        let bundle = Bundle.dl_buldle(forModule: module,bundleName:bundleName ?? module)
        if let image = UIImage(named: name, in: bundle, compatibleWith: nil) {
            return image
        }
        let scale = Int(UIScreen.main.scale)
        let sourceName = "\(name)@\(scale)x"
        print(sourceName)
        if let path = bundle?.path(forResource: sourceName, ofType: "png") {
